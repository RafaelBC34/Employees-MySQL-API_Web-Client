    ...
    axios.get(url)
        .then(response => {
            ...
            employees.forEach(employee => {
                ...
                liElem.appendChild(document.createTextNode(`Id: ${employee.id} | Name: ${employee.name} | Salary: $${employee.salary}`))
...
getEmployee_form.addEventListener('submit', function (evt) {
    ...
    axios.get(url + `/${userId}`)
        .then(response => {
            ...
            pElem.appendChild(document.createTextNode(`Id: ${employee.id} | Name: ${employee.name} | Salary: $${employee.salary}`))
...
postEmployee_form.addEventListener('submit', function (evt) {
    ...
    axios.post(url, jsonValues, {headers: axiosHeaders})
        .then(response => {
            ...
            let pElem = document.createElement('p')
            pElem.appendChild(document.createTextNode(`Id: ${addedEmployee.id} | Name: ${addedEmployee.name} | Salary: $${addedEmployee.salary}`))
...
updateEmployee_form.addEventListener('submit', function (evt) {
    ...
    if (!updateEmployee_name && !updateEmployee_salary) {
        updateEmployee_res.innerHTML = ''
        updateEmployee_res.appendChild(buildError({message: `Put a Name or a Salary to be updated for the user with id: ${updateEmployee_id}`}))
        return
    }
